...
The canonical documentation lives in [README.md](./README.md).
...
- `slurm-array` writes `job_manifest.json` and per-subject `status/` markers.
- `rerun-failed` rebuilds a job bundle containing only subjects marked `.failed`.
