...
def cmd_init(args):
    """Generate a starter fmriprep.ini in the target directory."""
    ...
    # Merge any user/system config so we can pre-fill values
    global_cfg = load_config()

    # Smart defaults based on CWD / target directory
    ...
    # Build the template, preferring global config values where available
    ...
    lines = [
        ...
        "#",
        "# Values here override user (~/.fmriprep.ini) and system defaults.",
...
Environment Variables:
  ...
  SLURM_* - Various SLURM variables for resource detection
    ...
    # init
    p_init = sub.add_parser("init", help="Generate a starter fmriprep.ini in the current (or specified) directory")
    p_init.add_argument("dir", nargs="?", default=".",
                        help="Target directory (default: current directory)")
    p_init.add_argument("--force", action="store_true",
                        help="Overwrite existing fmriprep.ini")
